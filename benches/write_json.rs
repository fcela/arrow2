...
fn add_benchmark(c: &mut Criterion) {
    (10..=18).step_by(2).for_each(|log2_size| {
        ...
        c.bench_function(&format!("ndjson write i32 2^{}", log2_size), |b| {
        ...
        c.bench_function(&format!("ndjson write utf8 2^{}", log2_size), |b| {
            b.iter(|| write_batch(&columns))
        });
        ...
        c.bench_function(&format!("ndjson write f64 2^{}", log2_size), |b| {
            b.iter(|| write_batch(&columns))
        });
    });
}
